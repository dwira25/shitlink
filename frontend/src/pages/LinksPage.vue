...
import { Copy, Edit, Plus, QrCode, Star, Trash2, X } from "@lucide/vue";
import LinkFormModal from "../components/LinkFormModal.vue";
import { bulkLinks, createLink, deleteLink, fetchLinks, fetchRatings, qrUrl, updateLink } from "../services/linkService";
import type { LinkForm, LinkRating, ShortLink } from "../types";
...
const ratings = ref<LinkRating[]>([]);
const ratingsLink = ref<ShortLink | null>(null);
...
async function openRatings(link: ShortLink) {
  ratingsLink.value = link;
  ratings.value = [];
  ratingsOpen.value = true;
  ratingsLoading.value = true;
  try {
    ratings.value = await fetchRatings(link.id);
  } catch {
    ratings.value = [];
  } finally {
...
<template>
  <section>
            ...
            <tr v-for="link in links" :key="link.id">
              ...
              <td class="px-4 py-3">
                <div class="flex gap-1">
                  ...
                  <button v-if="link.ratingEnabled" class="btn btn-soft h-9 w-9 p-0" title="Ratings" @click="openRatings(link)"><Star class="h-4 w-4" /></button>
    ...
    <div v-if="ratingsOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
      <div class="panel w-full max-w-lg p-6">
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-lg font-semibold">Ratings</h2>
            <p class="text-sm text-slate-500">{{ ratingsLink?.title }} · {{ ratingsLink?.slug }}</p>
          </div>
          <button class="btn btn-soft h-10 w-10 p-0" type="button" aria-label="Close" @click="ratingsOpen = false"><X class="h-4 w-4" /></button>
        </div>

        <div class="mt-4 max-h-96 overflow-y-auto">
          <p v-if="ratingsLoading" class="py-6 text-center text-slate-500">Memuat...</p>
          <p v-else-if="ratings.length === 0" class="py-6 text-center text-slate-500">Belum ada rating.</p>
          <ul v-else class="divide-y divide-line">
            <li v-for="r in ratings" :key="r.id" class="py-3">
              <div class="flex items-center justify-between">
                <span class="text-amber-500">{{ "★".repeat(r.score) }}{{ "☆".repeat(5 - r.score) }}</span>
                <span class="text-xs text-slate-400">{{ new Date(r.createdAt).toLocaleString() }}</span>
              </div>
              <p v-if="r.comment" class="mt-1 whitespace-pre-wrap text-sm text-slate-700">{{ r.comment }}</p>
            </li>
          </ul>
        </div>
      </div>
