...
describe("QR codes", () => {
  ...
  it("encodes the short URL, not the destination URL", async () => {
    const res = await authed("get", `/api/links/${customId}/qr?format=svg`);
    expect(res.text).toContain("promo2026");
    expect(res.text).not.toContain("example.com/promo-3");
  });

  it("rejects QR access to another user's link", async () => {
    const other = await request(app).post("/api/auth/login").send({ email: "nobody@test.local", password: "x" });
